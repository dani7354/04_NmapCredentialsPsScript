...
table.insert(fingerprints, {
  ...
  target_check = function (host, port, path, response)
    return http_auth_realm(response) == "SimpleWebservice"
  end,
  login_combos = {
    {username = "admin", password = "admin"},
    {username = "admin", password = "password"}
...
table.insert(fingerprints, {
  ...
  target_check = function (host, port, path, response)
    return response.status == 200
           and response.body
           and response.body:find("<title>Simple Login Page</title>", 1, true)
           and response.body:find('<form method="post" action="login.php">', 1, true)
  end,
  login_combos = {
  ...
  login_check = function (host, port, path, user, pass)
	...
	resp =  http_post_simple(host, port, url.absolute(path, "login.php"), nil, form)
	return resp.status == 302
